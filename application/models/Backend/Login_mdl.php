...
class Login_mdl extends CI_Model {
	...
	function admin_todays_notice(){
		$this->db->order_by('priority', 'asc');	
		$query = $this->db->get('notice_board');
		return $query->result_array();
	}
	
	function update_notice_board(){
	
		$noticeDate = strtotime(date('Y-m-d'));
		
		$old_notice_cnt = $this->input->post('h_old_notice_cnt');
		if(isset($old_notice_cnt) && $old_notice_cnt>0){
			$old_notice_ids = $this->input->post('old_notice_ids');
			foreach($old_notice_ids as $k){
				$noticeMsg = $this->input->post('old_notice_'.$k);
				$status = $this->input->post('old_status_'.$k);
				$priority = $this->input->post('old_priority_'.$k);
				$this->db->where('noticeId',$k); 
				$this->db->update('notice_board', array("noticeMsg"=>$noticeMsg, "status"=>$status, "priority"=>$priority, "noticeDate"=>$noticeDate));
			} 
		}
			
		$item_cnt = $this->input->post('h_item_cnt');		 
		if($item_cnt>0){
			for($i=1;$i<=$item_cnt;$i++){
				$noticeMsg = $this->input->post('notice_'.$i);
				$status = $this->input->post('status_'.$i);
				$priority = $this->input->post('priority_'.$i);
				$this->db->insert('notice_board', array("noticeMsg"=>$noticeMsg, "status"=>$status, "priority"=>$priority, "noticeDate"=>$noticeDate)); 
			}
		}		
			
		/*$query = $this->db->get_where('notice_board', array('noticeDate' =>$noticeDate));
		$count = $query->num_rows();
		if($count==0){
			$this->db->insert('notice_board', array("noticeMsg"=>$noticeMsg, "noticeDate"=>$noticeDate)); 
		}else{				
			$this->db->where('noticeDate',$noticeDate); 
			$this->db->update('notice_board', array("noticeMsg"=>$noticeMsg));
		}*/
		$this->session->set_flashdata('success', 'Notice has been updated!');
	}
	...
	function update_profile() {
		...
		$password = $this->input->post('password');
		$planningMinLinks = $this->input->post('planningMinLinks');
		...
		if(isset($_FILES['photo']['name']) && $_FILES['photo']['name']!=''){
			...
			if($_FILES['photo']['error']==0){
			 ...
			 if(isset($password) && $password!=''){
				...
				$data=array("planningMinLinks"=>$planningMinLinks,"username"=>$username,"name"=>$name, "email"=>$email, "image"=>$photo,"password"=>md5($password));
				 
			 }else{
				
				$data=array("planningMinLinks"=>$planningMinLinks,"username"=>$username,"name"=>$name, "email"=>$email, "image"=>$photo);
			...
			if(isset($password) && $password!=''){ 
				...
				$data=array("planningMinLinks"=>$planningMinLinks,"username"=>$username,"name"=>$name,"email"=>$email,"password"=>md5($password));	
			
			}else{
				
				$data=array("planningMinLinks"=>$planningMinLinks,"username"=>$username,"name"=>$name,"email"=>$email);	
