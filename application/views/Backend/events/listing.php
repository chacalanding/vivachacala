...
function delete_entry(val, pageName) {
	if (val != "") {
		var r = confirm("Are you sure want to delete it!");
		if (r == true) {
			window.location = "<?php echo $this->config->item('ajax_base_url').$this->config->item('admin_directory_name').'events/delete/'; ?>"+val+'?pageName='+pageName;
		}
	}
}
function update_toggle_swtich_values(eventId,column_name){
	if(eventId>0){
		var checkstatus=$('#toggle-event-'+column_name+eventId).prop('checked');
		if(checkstatus == true){
			var status=0;		
		}else{
			var status=1;		 
		}	
		$.ajax({url: "<?php echo base_url(). $this->config->item('admin_directory_name');?>events/update_event_status?eventId="+eventId+"&column_name="+column_name+"&status="+status, 
			beforeSend: function(){ 
				$('#spinner_'+column_name+'_'+eventId).html('<i class="fa fa-spinner fa-spin"></i>');
			},
			success: function(result){
				if(result=='success'){
					$('#spinner_'+column_name+'_'+eventId).html('');
				}
			}
		});
	} 
							...
							<table class="table table-striped " id="table_recordtbl">
								<thead>
									<tr>
										...
										<th>Title</th>
										<th>Cost</th>
										<th><?php if($pageName=='previous'){echo 'Record Sts';}else{echo 'Status';}?></th>
										<th>Action</th>
									</tr>
								</thead>
								<tbody>
									...
									foreach ($events_array as $row) { ?>
										<tr>
											<td><?php echo $i;?></td>
											<td style="font-weight:600;" nowrap="nowrap"><?php if(isset($row->eventTime) && $row->eventTime!='' && $row->eventTime>0){
											echo date('d M, Y',$row->eventTime).'<br>'.date('h:i A',$row->eventTime).' &ndash; '. $row->timeZone;
											
											}else{echo '&ndash;';}?></td>
											<td ><?php echo $row->eventTitle; ?><small><?php echo 'Type: '.$this->config->item('event_types_array_config')[$row->eventType]['name'];?></small></td>
											
											<td><small><?php if($row->costSts>0){ echo '$'.number_format($row->eventCost,2).''; }else{echo 'Free';}?></small></td>
											
										 
											<td>
											
											<?php if($pageName=='previous'){?>
											
												<?php if($row->showRecordSts==1){?>
													<label class="mstus accepted" style="padding:0px 10px;">Yes</label>
												<?php }else{ ?>
													<label class="mstus rejected" style="padding:0px 10px;">No</label>
												<?php } ?>
											<?php }else{ ?>
												<input <?php if(isset($row->isStatus) && $row->isStatus==0){?> checked="checked" <?php } ?> id="toggle-event-isStatus<?php echo $row->eventId;?>" onchange="return update_toggle_swtich_values('<?php echo $row->eventId;?>','isStatus');" data-toggle="toggle" data-size="mini" data-onstyle="success" data-offstyle="danger" data-on="Active" data-off="Inactive" type="checkbox">
												<span id="spinner_isStatus_<?php echo $row->eventId;?>"></span>
												<?php } ?>
											</td>
											<td nowrap="nowrap">
												<a class="btn btn-success btn-sm" href="<?php echo $this->config->item('ajax_base_url').$this->config->item('admin_directory_name').'events/edit/'.$row->encryptId;?>">Edit</a>
												<a style="margin-left:3px;" onclick="return delete_entry('<?php echo $row->encryptId; ?>','<?php echo $pageName;?>');" class="btn btn-danger btn-sm"><i class="fa fa-trash"></i></a>
											</td>
										</tr>
									<?php $i++;
									} ?>
